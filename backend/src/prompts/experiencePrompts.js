...
const JOB_KEYEXP_META = {
  ...
  pm: {
    unit: '의사결정',
    guide: '각 핵심 경험을 하나의 프로덕트 의사결정 단위로 추출: 사용자·비즈니스 문제에서 어떤 가설을 세웠고, 무엇을 하기로 결정했으며, 어떤 대안을 왜 기각했고, 가설을 어떤 데이터·인터뷰·실험으로 검증해 무엇을 판단했는지가 드러나야 합니다. hypothesis는 기대한 변화와 관찰할 사용자 행동·성과가 연결된 검증 가능한 문장으로 작성하세요. learning에는 단순 소감이 아니라 검증 전 믿음과 달라진 점, 다음 결정에서 바꿀 원칙을 작성하세요. impact/effort는 원본 근거로 1~5 정수를 반드시 추정해 채우세요(우선순위 매트릭스 좌표로 그려짐) — 근거가 전혀 없을 때만 비웁니다.',
    schema: '{ "hypothesis": "문제에서 도출한 검증 가능한 가설 1문장 (변화→예상 행동·성과)", "decision": "내린 핵심 결정 한 문장", "alternatives": "고려한 대안과 기각 이유 1문장", "stakeholders": "설득·협업한 이해관계자와 방법 1문장", "validation": "가설 검증 방법·기준·근거 (데이터·인터뷰·실험)", "impact": "1~5 정수 — 이 결정의 비즈니스 임팩트 크기", "effort": "1~5 정수 — 투입된 리소스·난이도" }',
  },
  designer: {
    unit: '개선 반복',
