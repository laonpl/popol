...
- code_snippets는 diff에 코드가 있으면 가장 핵심적인 것 1~2개만 발췌하십시오. why는 2문장 내외로 작성하십시오. 첫 문장은 핵심 로직이 입력·상태를 어떻게 처리하는지, 둘째 문장은 이 방식을 선택한 이유와 해결 효과를 설명하십시오. 단순히 '코드를 추가함'처럼 코드를 되풀이하거나 diff에 없는 효과를 추측하지 마십시오.
- troubleshooting_snippets의 solution도 2문장 내외로 작성하십시오. 문제의 기술적 원인과 코드에서 확인되는 해결 방식·재발 방지 효과가 자연스럽게 이어져야 합니다.
...

{
  ...
  "code_snippets": [
    { "file": "변경된 파일 경로 (예: src/auth/middleware.ts)", "code": "제공된 diff에서 발췌한 실제 핵심 변경 코드 3~10줄. diff에 없으면 이 항목 생략", "why": "핵심 로직의 처리 방식 → 선택 이유와 해결 효과가 이어지는 구체적인 2문장 설명" }
  ],
  ...
  "troubleshooting_snippets": [
    { "issue": "발생한 문제/버그 상황 한 줄 (예: 동시 요청 시 중복 결제 발생)", "file": "관련 파일(선택)", "code": "이 문제를 해결한 실제 diff 코드 3~10줄", "solution": "문제의 기술적 원인 → 코드에서 확인되는 해결 방식과 효과가 이어지는 구체적인 2문장 설명" }
  ],
