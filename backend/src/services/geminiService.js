...
import { groundPmEvidence } from './pmEvidence.js';
...
export async function generateDraftAnalysis(content, jobCategory = 'common', careerStage = 'first', interviewMode = 'basic') {
  ...
  return groundMarketerEvidence(groundPmEvidence(sanitizeArtifactsDeep(groundAnalysisMetrics(hydrated, contentText)), content), content);
}
...
export async function analyzeExperience(content, keyExperienceCount = 3, reviewedMoments = null, jobCategory = 'common', careerStage = 'first') {
  ...
  const result = {
    projectOverview: resultProjectOverview,
    ...(['pm', 'marketer'].includes(jobCategory) && (keyExperiences.length < targetCount || keyExperiences.some(item => item._extractionFailed)) ? { _fallback: true, _fallbackReason: 'key_experience_extraction_failed' } : {}),
  ...
  return groundMarketerEvidence(groundPmEvidence(sanitizeArtifactsDeep(groundAnalysisMetrics(result, contentText)), content), content);
}
...
export async function refineKeyExperience(currentExp, freeFormText) {
  ...
  // A free-form refinement has no original files to re-check. Keep the existing
  // grounded PM record intact; new source evidence is generated by full reanalysis.
  if (currentExp?.jobData?.pmEvidenceVersion === 1) {
    const previous = currentExp.jobData;
    merged.jobData = { ...merged.jobData, pmEvidenceVersion: 1,
      pmEvidence: previous.pmEvidence || [], pmMetrics: previous.pmMetrics || [],
      pmWorkProducts: previous.pmWorkProducts || {}, pmWorkProductsVersion: previous.pmWorkProductsVersion || 0,
      impact: previous.impact || '', effort: previous.effort || '', priorityQuote: previous.priorityQuote || '',
    };
  }
  if (currentExp?.jobData?.marketerEvidenceVersion) {
    const previous = currentExp.jobData;
    merged.jobData = { ...merged.jobData, marketerEvidenceVersion: previous.marketerEvidenceVersion,
      marketerEvidence: previous.marketerEvidence || [], marketerMetrics: previous.marketerMetrics || [],
      marketerWorkProducts: previous.marketerWorkProducts || {}, marketerWorkProductsVersion: previous.marketerWorkProductsVersion || 0,
    };
  }
  merged.artifactBindings = currentExp.artifactBindings || [];
