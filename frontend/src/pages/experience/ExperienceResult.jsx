...
import { roleSourceText, roleArtifacts, caseArtifacts } from '../../utils/roleArtifacts';
...
import { buildPmEvidenceModel } from '../../utils/pmEvidence';
import { buildMarketerEvidenceModel } from '../../utils/marketerEvidence';
...
/* ══════════════════════════════════════════════════════════
   기획/PM 전용 — 프로젝트 맥락 → 의사결정 기록 → 원문 근거와 관찰 결과.
   요약 편집은 jobData.pmEvidence에 저장하고 인용 원문은 변경하지 않는다.
   ══════════════════════════════════════════════════════════ */

const pmFileExt = name => String(name || '').split('.').pop().toUpperCase().slice(0, 5);
const pmSafeUrl = value => /^https?:\/\//i.test(String(value || '').trim()) ? String(value).trim() : `https://${String(value || '').trim()}`;

export function PmProfileSidebar({ cs, sr, evidence, setField, setMeta, onPatchSr, role = 'pm' }) {
  const [uploading, setUploading] = useState(false);
  const [link, setLink] = useState('');
  const fileRef = useRef(null);
  const files = roleArtifacts(sr);
  const allRecords = evidence.cases.flatMap(item => item.records);
  const allWork = evidence.cases.flatMap(item => Object.values(item.workProducts || {}).flat());
  const discovery = allRecords.find(item => item.dimension === (role === 'marketer' ? 'targeting' : 'discovery'))?.claim || '';
  const delivery = allRecords.find(item => item.ownership) || allWork.find(item => item.ownership || item.owner);
  const learning = [...allRecords].reverse().find(item => item.dimension === 'learning')?.claim || '';
  const updateFiles = next => onPatchSr({ ...sr, deliverables: next, pmFiles: [], additionalMaterials: (sr.additionalMaterials || []).filter(row => next.some(file => file.id === row.assetId)) });
  const pickFiles = async event => {
    ...
    setUploading(true);
    const added = [];
    const materials = [...(sr.additionalMaterials || [])];
    for (const file of picked) {
      try {
        const uploaded = await uploadDocumentFile(file);
        const asset = { id: uid(), kind: 'file', name: uploaded.name || file.name, url: uploaded.url, size: uploaded.size || file.size, ext: String(file.name).split('.').pop().toLowerCase() };
        added.push(asset);
        const remaining = Math.max(0, 60000 - materials.reduce((sum, row) => sum + (row.text?.length || 0), 0));
        if (remaining) {
          try {
            const form = new FormData(); form.append('file', file);
            const imported = await importFileUpload(form);
            const extracted = imported?.imported?.content || '';
            if (extracted.trim()) {
              const text = extracted.slice(0, Math.min(20000, remaining));
              materials.push({ assetId: asset.id, name: asset.name, text });
              if (text.length < extracted.length) toast('추가 자료의 일부 본문만 분석에 포함됩니다. 필요한 페이지로 나누어 올리면 더 정확합니다.');
            } else toast('원본은 보관했습니다. 본문을 읽지 못해 자동 추출 근거에는 포함하지 않습니다.');
          } catch { toast('원본은 보관했습니다. 본문 추출에 실패해 파일 연결과 설명 작성만 가능합니다.'); }
        } else toast('추가 본문 한도에 도달해 원본 파일만 보관합니다.');
      } catch (error) {
        toast.error(error?.response?.data?.error || `'${file.name}' 업로드에 실패했어요`);
      }
    }
    if (added.length) onPatchSr({ ...sr, deliverables: [...files, ...added], pmFiles: [], additionalMaterials: materials });
    setUploading(false);
  };
  const addLink = event => {
    event.preventDefault();
    try {
      const url = pmSafeUrl(link);
      const parsed = new URL(url);
      if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error('invalid');
      updateFiles([...files, { id: uid(), kind: 'link', name: parsed.hostname.replace(/^www\./, ''), url }]);
      setLink('');
    } catch { toast.error('올바른 웹 링크를 입력해 주세요'); }
  };
  return (
    <aside className="pm-profile-sidebar">
      <div className="pm-profile-sticky">
        <div className="pm-project-kicker"><span>{role === 'marketer' ? 'MARKETING' : 'PRODUCT MANAGER'}</span><span>CASE STUDY</span></div>
        <AutoText prose value={cs.title} onChange={value => setField('title', value)} placeholder={role === 'marketer' ? '어떤 캠페인·브랜드의 작업인가요?' : '어떤 제품의 문제를 해결했나요?'} className="pm-profile-title" />
        <AutoText prose value={cs.summary} onChange={value => setField('summary', value)} placeholder={role === 'marketer' ? '타깃 고객과 전달하려던 메시지를 한 문장으로' : '대상 사용자와 해결하려던 문제를 한 문장으로'} className="pm-profile-summary" />
        <div className="pm-profile-meta">
          {[['role', '내 역할과 책임'], ['duration', '프로젝트 기간'], ['team', '함께한 팀']].map(([key, label]) => <div key={key}><span>{label}</span><AutoText value={cs.meta?.[key] || ''} onChange={value => setMeta(key, value)} placeholder="직접 입력" className="pm-profile-meta-value" /></div>)}
        </div>

        <div className="pm-judgment-profile">
          <div className="pm-side-heading"><span>판단 프로필</span><small>{evidence.legacy ? '원문 대조 전' : '자료 기반'}</small></div>
          {[[role === 'marketer' ? '고객을 이해한 방식' : '문제를 읽은 방식', discovery], ['직접 책임진 범위', delivery?.ownership || delivery?.owner], ['판단이 바뀐 지점', learning]].map(([label, value]) => <div className="pm-judgment-item" key={label}><span>{label}</span><p>{value || '연결된 자료에서 아직 확인되지 않았습니다.'}</p></div>)}
        </div>

        <div className="pm-source-vault">
          <div className="pm-side-heading"><span>산출물 보관함</span><small>{files.length}개</small></div>
          {files.length > 0 ? <div className="pm-source-files">{files.map(file => <div key={file.id || file.url} className="pm-source-file"><a href={file.url} target="_blank" rel="noreferrer"><span>{file.kind === 'link' ? <Link2 size={14} /> : pmFileExt(file.name) || 'FILE'}</span><b title={file.name}>{file.name || '산출물 열기'}</b></a><button type="button" onClick={() => updateFiles(files.filter(item => item.url !== file.url))} aria-label={`${file.name || '산출물'} 삭제`}><Trash2 size={12} /></button></div>)}</div> : <p className="pm-source-empty">{role === 'marketer' ? '광고 시안, 콘텐츠, 미디어 플랜, 성과 리포트를 보관하세요.' : 'PRD, 회의록, 리서치, 지표 리포트를 올리면 판단과 함께 보여줍니다.'}</p>}
          <input ref={fileRef} className="hidden" type="file" multiple accept=".pdf,.ppt,.pptx,.hwp,.hwpx,.doc,.docx,.xls,.xlsx,.csv,.tsv,.txt,.md,.zip,.png,.jpg,.jpeg,.webp,.gif" onChange={pickFiles} />
          <button type="button" className="pm-add-file print:hidden" onClick={() => fileRef.current?.click()} disabled={uploading}>{uploading ? <Loader2 size={13} className="animate-spin" /> : <Plus size={14} />}{uploading ? '올리는 중…' : '파일 추가'}</button>
          {role === 'marketer' && <div className="rpm-source-advice print:hidden"><b>이 자료에서 직무의 핵심을 추출합니다</b>광고·분석 CSV → 측정 조건 / 소재·카피 → 표현 선택 / 미디어 플랜 → 채널·예산 / CRM 기록 → 발송 조건·개선</div>}
          <p className="pm-source-empty print:hidden">파일 본문을 읽어 저장합니다. 저장 후 ‘산출물 다시 추출’을 실행하면 반영됩니다. 링크는 원본 열기용이며 내용을 자동 분석하지 않습니다.</p>
          <form className="pm-add-link print:hidden" onSubmit={addLink}><Link2 size={13} /><input value={link} onChange={event => setLink(event.target.value)} placeholder="Notion · Figma · Drive" /><button disabled={!link.trim()}>추가</button></form>
        </div>
      </div>
    </aside>
  );
...
export function PmDoc({ exp, cs, sr, setField, setMeta, onPatchSr }) {
  const sourceText = getRawMaterialText(exp) || (typeof exp?.content === 'string' ? exp.content : Object.values(exp?.content || {}).filter(v => typeof v === 'string').join('\n\n'));
  const evidence = buildPmEvidenceModel(sr, { sourceText: roleSourceText(sr, sourceText) });
  const patchRecord = (caseIndex, recordIndex, claim) => {
    const keyExperiences = (sr.keyExperiences || []).map((item, index) => index !== caseIndex ? item : {
      ...item, jobData: { ...item.jobData, pmEvidence: (item.jobData?.pmEvidence || []).map((record, ri) =>
        ri !== recordIndex ? record : { ...record, claim, userEdited: true }) },
    });
    onPatchSr({ ...sr, keyExperiences });
  };
  return (
    <div className="pm-workspace pm-page-grid">
      <PmProfileSidebar cs={cs} sr={sr} evidence={evidence} setField={setField} setMeta={setMeta} onPatchSr={onPatchSr} />
      <main className="pm-page-main">
        <PmEvidenceCaseStudy sr={sr} sourceText={sourceText} onRecordChange={patchRecord} onChange={onPatchSr} />
        <details className="pd-project-notes" open={(cs.body || []).some(segment => segment.content?.trim()) || undefined}>
          <summary>프로젝트 추가 설명 <span>공통 배경 · 자유 편집 본문</span></summary>
          <p>기존에 작성한 본문과 이미지를 그대로 보존한 영역입니다. 위 핵심 경험에 공통으로 필요한 배경을 보충하세요. 특정 경험의 원본 파일은 해당 경험의 ‘근거 자료’에서 연결합니다.</p>
          <CaseBody body={cs.body || []} onChange={next => setField('body', next)} />
        </details>
      </main>
    </div>
...
export function MarketingProjectDesk({ cs, sr, evidence, setField, setMeta, onPatchSr }) {
  return <><header className="marketing-print-context"><h2>{cs.title || '캠페인 프로젝트'}</h2>{cs.summary && <p>{cs.summary}</p>}<dl>{[['role', '내 역할'], ['duration', '기간'], ['team', '팀']].filter(([key]) => cs.meta?.[key]).map(([key, label]) => <div key={key}><dt>{label}</dt><dd>{cs.meta[key]}</dd></div>)}</dl></header><details className="marketing-project-desk print:hidden"><summary><b>{cs.title || '캠페인 프로젝트'}</b><span>{[cs.meta?.role, cs.meta?.duration].filter(Boolean).join(' · ')}</span><small>프로젝트 정보 · 자료 보관함 열기</small><ChevronDown size={14} /></summary><PmProfileSidebar role="marketer" cs={cs} sr={sr} evidence={evidence} setField={setField} setMeta={setMeta} onPatchSr={onPatchSr} /></details></>;
}

export function MarketerDoc({ cs, sr = {}, sourceText, setField, setMeta, onPatchSr }) {
  const evidence = buildMarketerEvidenceModel(sr, { sourceText: roleSourceText(sr, sourceText) });
  const connectedImages = new Set(evidence.cases.flatMap(item => caseArtifacts(sr, item).filter(file => file.image).map(file => file.url)));
  const previousImages = (cs.keyExps || []).map(item => ({ ...item, images: (item.images || []).filter(image => !connectedImages.has(image.url)) })).filter(item => item.images.length);
  return <div className="marketing-workspace">
    <MarketingProjectDesk cs={cs} sr={sr} evidence={evidence} setField={setField} setMeta={setMeta} onPatchSr={onPatchSr} />
    <main className="marketing-main">
      <MarketerEvidenceCaseStudy sr={sr} sourceText={sourceText} onChange={onPatchSr} />
      <details className="marketing-project-notes" open={previousImages.length > 0 || (cs.body || []).some(segment => segment.content?.trim()) || undefined}>
        <summary>프로젝트 추가 설명 · 기존 이미지</summary><p>캠페인에 공통으로 필요한 배경과 기존에 작성한 내용을 보존합니다. 위 작업물 영역에 표시한 이미지는 여기서 반복하지 않습니다.</p>
        {previousImages.map((item, i) => <div key={item.id || i}><h4>{item.title}</h4>{item.images.map((image, index) => <a key={index} href={image.url} target="_blank" rel="noreferrer"><img src={image.url} alt={image.caption || item.title || '기존 작업 이미지'} style={{ maxWidth: '100%', maxHeight: 440, objectFit: 'contain', margin: '14px 0' }} /></a>)}</div>)}
        <CaseBody body={cs.body || []} onChange={next => setField('body', next)} />
      </details>
    </main>
  </div>;
    ...
    <div className={`min-h-screen ${isMarketer ? 'bg-surface-50 print:bg-white' : 'bg-white'}`}>
      ...
      <div className="sticky top-0 z-20 border-b border-surface-200 bg-white/90 backdrop-blur print:hidden">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            {(isPmJob || isMarketer) && <button type="button" onClick={goEnhanceDraft} className="hidden sm:inline-flex rounded-lg border border-primary-200 bg-white px-3.5 py-2 text-[13px] font-bold text-primary-700">산출물 다시 추출</button>}
      ...
      <article className={isMarketer ? 'mx-auto max-w-[1080px] px-5 py-9 sm:px-10 print:max-w-none print:p-0' : isPmJob ? 'px-4 sm:px-6 xl:px-8 py-7 sm:py-9' : isCommonJob ? 'mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-12' : 'max-w-6xl mx-auto px-5 sm:px-8 py-7 sm:py-9'}>
        ...
        {isMarketer ? (
          <MarketerDoc
            cs={cs}
            onPatchSr={nextSr => { setExp(prev => ({ ...(prev || {}), structuredResult: nextSr })); setDirty(true); }}
            kit={kitView}
            sr={exp?.structuredResult}
            sourceText={getRawMaterialText(exp)}
            setField={setField}
            setMeta={setMeta}
            setKeyExp={setKeyExp}
            addKeyExp={addKeyExp}
