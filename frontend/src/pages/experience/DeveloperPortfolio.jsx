...
import {
  ...
  PenLine, Check, X, Layers, Cpu, Plus, Trash2, GitCommit, Users,
...
/* ── 시스템 아키텍처 다이어그램 (박스 + 화살표 연결선, 순수 SVG · 자유 배치) ── */
const FS_LABEL = 13, FS_TECH = 10.5, MAX_TEXT_W = 186;
const ROW_GAP = 62, COL_GAP = 34, PAD = 18, PAD_X = 16, PAD_Y = 12;
const LINE_LABEL = 17, LINE_TECH = 14;

// 노드별 텍스트 줄바꿈 + 박스 크기 (커스텀 w/h 우선, 없으면 내용에 맞춰 자동)
function computeNodeMetrics(nodes) {
  const metrics = {};
  nodes.forEach(n => {
...
function autoLayoutPositions(nodes, metrics) {
...
const hasXY = (n) => Number.isFinite(n?.x) && Number.isFinite(n?.y);
...
function buildFallbackDiagram(techs) {
...
function ArchitectureDiagram({ diagram, editable = false, canvas = null, onNodeMove }) {
...
function CodeSnippet({ file, code, lead, explanation }) {
  return (
    <div className="mb-3">
...
function GitProjectCard({ exp, index }) {
...
/* ── GitHub 커밋 기여도 ── */
function ContributionStats({ stats }) {
