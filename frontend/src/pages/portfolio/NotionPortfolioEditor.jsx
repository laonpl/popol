        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-surface-100 rounded-xl p-1 gap-0.5">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-medium transition-all ${analysisMode
                ...
                <div key={cat}>
                  ...
                  {items.map((item, i) => (
                    <div key={i} className="flex items-start gap-2 py-1">
                      <div className={`w-4 h-4 rounded border-2 flex-shrink-0 flex items-center justify-center mt-0.5 ${item.passed ? 'bg-green-500 border-green-500' : 'bg-white border-gray-300'
                        }`}>
  ...
  return (
    <>
      <button
        ...
        onClick={handleClick}
        disabled={loading}
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${show ? 'bg-indigo-100 text-indigo-700 ring-1 ring-indigo-300' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'
          } disabled:opacity-50`}
      >
          ...
          <div className="flex items-center gap-2">
            {jobAnalysis && (
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-medium transition-all ${showTailor
                    ...
                    {(() => {
                      ...
                      return availableCount > 0 && (
                        <button
                          ...
                          disabled={allApplied}
                          className={`w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold transition-colors ${allApplied
                        ...
                        <div key={key} className="rounded-xl border border-gray-200 overflow-hidden bg-white">
                          <div className="flex items-center gap-2 px-3 py-2 bg-surface-50 border-b border-gray-100">
                            ...
                            <button
                              ...
                              disabled={isApplied}
                              className={`flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-medium transition-colors ${isApplied
                                  ? 'bg-green-100 text-green-700 cursor-default'
                                  : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-200'
                                }`}
                            >
                  ...
                  <div key={i} className="p-4 bg-surface-50 rounded-lg border border-surface-100 relative group/goal">
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <select value={g.status || 'planned'} onChange={e => updateArrayItem('goals', i, { status: e.target.value })}
                        className={`px-2 py-0.5 rounded text-xs outline-none border-none cursor-pointer ${g.status === 'done' ? 'text-green-600' : g.status === 'ing' ? 'text-blue-600' : 'text-gray-400'
                          }`}>
