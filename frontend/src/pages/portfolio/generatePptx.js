...
function txt(slide, text, x, y, w, h, opts = {}) {
  ...
  const runOpts = { fontFace: ff };
  if (opts.fontSize) runOpts.fontSize = opts.fontSize;
  if (opts.bold) runOpts.bold = opts.bold;
  ...
  if (opts.align) paraOpts.align = opts.align;
  else paraOpts.align = 'left';
  if (opts.valign) paraOpts.valign = opts.valign;
  if (opts.isTextBox != null) paraOpts.isTextBox = opts.isTextBox;
  if (opts.paraSpaceAfter != null) paraOpts.paraSpaceAfter = opts.paraSpaceAfter;
...
function buildProfile(prs, p, t) {
  ...
  // 기업 맞춤형 경험 필터링 및 재작성 로직 (PPT용)
  let exps = (p.experiences || []).slice(0, 3);
  const tailored = p.jobAnalysis?.tailoredPortfolio?.recommendedExperiences;
  if (tailored && tailored.length > 0) {
    exps = tailored.map(te => {
      const original = (p.experiences || []).find(e => e.title === te.title) || {};
      return {
        ...original,
        organization: original.organization || te.title,
        role: te.tailoredRole || original.role || te.reason,
        description: te.tailoredDescription || original.description
      };
    }).slice(0, 3);
  }
